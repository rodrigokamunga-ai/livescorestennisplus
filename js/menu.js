(() => {
  ...
  const MenuApp = (() => {
    ...
    function setWelcomeLocalSession() {
    ...
    function init() {
      ...
      localSessionMode = hasLocalSession();
      console.log("[menu] sessão local:", localSessionMode);

      // ✅ Modo biometria: não depende de Firebase Auth para permanecer no menu
      if (localSessionMode) {
        setWelcomeLocalSession();

        // Em modo biométrico, não força logout nem redireciona.
        // Apenas libera o menu com a sessão local.
        return;
      }

      // ✅ Modo normal Firebase Auth
  ...
  const MenuApp = (() => {
    ...
    function init() {
      ...
      console.log("[menu] sessão local:", localSession);

      auth.onAuthStateChanged(async (user) => {
        console.log("[menu] onAuthStateChanged user:", user ? user.uid : null);

        // Aceita Firebase Auth OU sessão local da biometria
        if (!user && !localSession) {
          goLogin();
          return;
        }

        // Se não houver user Firebase, tenta seguir com sessão local
        if (!user && localSession) {
          if (el.welcomeTitle)       el.welcomeTitle.textContent = "Bem-vindo";
          if (el.welcomeText)        el.welcomeText.textContent = "Acesso liberado pela biometria.";
          if (el.headerUserTitle)    el.headerUserTitle.textContent = "Menu";
