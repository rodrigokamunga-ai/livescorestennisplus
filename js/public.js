(() => {
  ...
  const PublicApp = (() => {
    ...
    function renderFinalizedCard(match) {
      ...
      if (tournament) footerRow1.push(`<span class="footer-item">Torneio: <strong>${tournament}</strong></span>`);
      if (stage) footerRow1.push(`<span class="footer-item">Fase: <strong>${stage}</strong></span>`);
      if (duration) footerRow2.push(`<span class="footer-item">Duração: <strong>${duration}</strong></span>`);

      return ` <article class="public-card match-board compact-match-board" data-status="${status}"> <div class="match-board-top compact-top"> ${category ? `<div class="match-chip">${category}</div>` : ""} <div class="match-status ${U.statusClass(status)}">${U.statusLabel(status)}</div> </div> ${buildSetHead(setColumns)} ${buildPlayerRow(team1Html, setColumns, ptDisp.p1, 1, score, winnerPos === 1, isWO, true, winnerPos)} ${buildPlayerRow(team2Html, setColumns, ptDisp.p2, 2, score, winnerPos === 2, isWO, true, winnerPos)} <div class="match-footer compact-footer match-footer-finalized"> ${footerRow1.length ? `<div class="match-footer-finalized-row">${footerRow1.join('<span class="footer-sep">-</span>')}</div>` : ""} ${footerRow2.length ? `<div class="match-footer-finalized-row">${footerRow2.join('<span class="footer-sep">-</span>')}</div>` : ""} </div> </article> `;
    }
