...
function scrollToStatsServeArea(server) {
  ...
  requestAnimationFrame(() => {
    ...
    function flashStatsButton(btn, playerPos) {
      ...
      void btn.offsetWidth; // força o repaint para a animação reiniciar
    ...
    function bindStatsButtonFlash() {
      document.querySelectorAll("#statsControlPanel .play-btn").forEach((btn) => {
        btn.addEventListener("click", () => {
          ...
          // Botões do Jogador 1 começam com j1_
...
(() => {
  ...
  const PublicApp = (() => {
    ...
    function renderStatistics(match) {
      ...
      const fmtPair = (a, b) => ` <div class="stat-values"> <span>${U.escapeHtml(String(a ?? 0))}</span> <span>${U.escapeHtml(String(b ?? 0))}</span> </div> `;
    
      const fmtTextPair = (a, b) => ` <div class="stat-values"> <span>${U.escapeHtml(String(a ?? "0"))}</span> <span>${U.escapeHtml(String(b ?? "0"))}</span> </div> `;
      ...
      // Pontos na rede: usa netWon/netLost, com fallback para soma dos golpes na rede
